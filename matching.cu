...
__global__ void FindMaxCorr(float *corrData, SiftPoint *sift1, SiftPoint *sift2, int numPts1, int corrWidth,
                            int siftSize) {
...
__global__ void FindMaxCorr10(SiftPoint *sift1, SiftPoint *sift2, int numPts1, int numPts2) {
  ...
  // number of valid points in this block (tail handling)
  int n1_block = max(0, min(M7W, numPts1 - bp1));
  ...
  if (ty == 0 && tx < n1_block) {
    ...
    for (int y = 0; y < M7H / M7R; ++y) {
      int pos = y * M7W + tx;
      // If your scratch only stores rows that exist, you can keep this.
      // Otherwise you may want to bound 'pos' to the rows you actually produced.
      if (best_idx != indices[pos]) {
        float s = scores1[pos];
        if (s > max_score_v) {
          sec_score_v = fmaxf(max_score_v, sec_score_v);
          max_score_v = s;
          best_idx    = indices[pos];
        } else if (s > sec_score_v) {
          sec_score_v = s;
    ...
    // Clamp best_idx before using it to read sift2
    int safe_idx = (best_idx >= 0 && best_idx < numPts2) ? best_idx : 0;

    sift1[bp1 + tx].score      = max_score_v;
    sift1[bp1 + tx].match      = best_idx;              // keep the raw index
    sift1[bp1 + tx].match_xpos = sift2[safe_idx].xpos;  // safe read
    sift1[bp1 + tx].match_ypos = sift2[safe_idx].ypos;  // safe read
    sift1[bp1 + tx].ambiguity  = sec_score_v / (max_score_v + 1e-6f);
  }
}
...
double FindHomography(SiftData &data, float *homography, int *numMatches, int numLoops, float minScore,
...
double MatchSiftData(SiftData &data1, SiftData &data2) {
  ...
  auto ans1 = cudaDeviceSynchronize();
  ...
  auto ans = cudaDeviceSynchronize();
  if (ans != cudaSuccess) {
    return -1.0;
  }
#endif
  ...
  if (data1.h_data != NULL) {
    ...
    safeCall(
      cudaMemcpy2D(h_ptr, sizeof(SiftPoint), d_ptr, sizeof(SiftPoint), 5*sizeof(float), data1.numPts,
